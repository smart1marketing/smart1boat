    ...
    "schema": {
        ...
        "properties": {
            ...
            "market_profile": {
                ...
                "required": ["estimated_population_low","estimated_population_base","estimated_population_high","estimated_households_low","estimated_households_base","estimated_households_high","estimated_boat_owner_households_low","estimated_boat_owner_households_base","estimated_boat_owner_households_high","estimated_ownership_rate_low","estimated_ownership_rate_base","estimated_ownership_rate_high","confidence","assumptions"]
            },
            "water_access_overview": {
                ...
                "items": {
                    ...
                    "required": ["waterway","type","communities_served","boating_fit","seasonality","confidence"]
                }
            },
            "geofence_locations": {
                "type": "array",
                "items": {
                    ...
                    "properties": {
                        ...
                        "recommended_method": {"type": "string", "enum": ["location_lookback", "real_time_proximity", "both"]},
                        "recommended_radius_miles": {"type": "number"},
                        "audience_reason": {"type": "string"},
                        "best_message": {"type": "string"},
                        "confidence": {"type": "string", "enum": ["low", "medium", "high"]}
                    },
                    "required": ["name","city_state","address","category","waterway_or_market","priority","recommended_method","recommended_radius_miles","audience_reason","best_message","confidence"]
                }
            },
            "zip_and_community_targets": {
                ...
                "items": {
                    ...
                    "required": ["community","state","zip_codes","priority","reason","recommended_audiences"]
                }
            },
        ...
        "required": ["market_summary","methodology_note","market_profile","water_access_overview","geofence_locations","zip_and_community_targets","audience_segments","campaign_plan","sales_opportunities","disclaimer"]
    },
    "strict": True
}
...
def clean_payload(data: dict[str, Any]) -> dict[str, Any]:
    fields = [
        "dealer_name", "website", "dealer_zip", "target_radius", "target_markets",
        "boat_types", "new_used", "campaign_objective", "monthly_budget",
        "seasonality", "known_waterways", "known_competitors", "contact_name",
...
Return 18-30 geofence locations when the market size reasonably supports it. Include a mix of boating access, marinas, storage/service, competitors, marine retail, and event venues. Prioritize locations inside the target radius and clearly lower confidence for uncertain locations.
The ownership rates must be decimals expressed as percentages, such as 7.5 for 7.5 percent—not 0.075.
...
def send_webhook(payload: dict[str, Any], report: dict[str, Any] | None, status: str) -> None:
    if not WEBHOOK_URL:
        return
    body = {
        **payload,
        ...
        "estimated_boat_owner_households_base": (report or {}).get("market_profile", {}).get("estimated_boat_owner_households_base"),
...
def analyze():
    try:
        ...
        try:
            ...
            pass
        return jsonify({"ok": False, "error": "The report could not be generated. Check the server configuration and try again."}), 500
